...
reverse -c [fileName]
- Inverte le linee di input lette da fileName e le mostra
  sullo standard output.
- Se non si specifica alcun file di input, reverse inverte
  lo standard input.
- Con l'opzione -c, reverse inverte anche i caratteri su
  ogni linea.
*/
...
int pass1(void) { /* Perform first scan through file */
  ...
  if(standardInput) { /* Read from standard input */
    ...
    tmpfd = open(tmpName, O_CREAT | O_RDWR | O_EXCL, 0600);
