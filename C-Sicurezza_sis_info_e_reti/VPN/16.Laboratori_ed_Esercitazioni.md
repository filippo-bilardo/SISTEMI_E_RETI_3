...
Questo capitolo fornisce 9 laboratori pratici progressivi per implementare e testare VPN in ambiente controllato. Ogni lab è disponibile come file separato con obiettivi, prerequisiti, istruzioni step-by-step e verifiche dettagliate.
...
## Lab 9: VPN IPsec Site-to-Site con Router Cisco (Packet Tracer)

**➡️ [Vai al Lab 9 completo](Lab9_Cisco_IPsec_VPN_Packet_Tracer.md)**

### Obiettivi
- Creare topologia VPN in Cisco Packet Tracer
- Configurare IPsec site-to-site con router Cisco IOS
- Implementare IKEv1 con pre-shared key
- Configurare crypto map, transform set e ACL
- Troubleshooting VPN Cisco con comandi diagnostici

### Topology
```
Site HQ (192.168.1.0/24) ←→ Internet ←→ Site Branch (192.168.2.0/24)
...
- Posizionamento dispositivi (Router 2911, Switch, PC, Cloud)
- Cablaggio completo
- Configurazione Cloud per simulare Internet

**Parte 2**: Configurazione base router
- Indirizzamento IP WAN e LAN
- Routing statico
- Configurazione PC
- Verifica connettività pre-VPN

**Parte 3**: Configurazione VPN IPsec (IKEv1)
- ISAKMP policy (Phase 1): encryption, authentication, DH group
- IPsec transform set (Phase 2): ESP-AES-256, SHA-256
- Access-list per traffico interessante (speculari)
- Crypto map e applicazione su interfaccia WAN

**Parte 4**: Testing e verifica
- Generazione traffico interessante (ping)
- Verifica tunnel: `show crypto isakmp sa` e `show crypto ipsec sa`
- Analisi contatori pacchetti cifrati/decifrati
- Debug crypto (ISAKMP e IPsec)

**Parte 5**: Troubleshooting
- Tunnel non si stabilisce (checklist completa)
- Tunnel attivo ma traffico non passa
- ACL speculare errata
- Pre-shared key mismatch
- Comandi reset: `clear crypto isakmp/sa`

**Parte 6**: Configurazioni avanzate
- IKEv2 (se supportato)
- NAT Traversal
- Logging eventi crypto
- Multiple crypto maps (più tunnel)

**Parte 7**: Scenari ed esercizi
- Backup tunnel (ridondanza)
- Subnet multiple
- QoS su traffico VPN
- Monitoring con SNMP
...
- Analisi pacchetti ESP cifrati
- Confronto traffico cifrato vs chiaro

**Parte 9**: Salvataggio e condivisione
- Esportazione configurazioni
- Template riutilizzabili

**Quick Command Reference**: comandi essenziali Cisco IPsec

**Domande di verifica**: 14 domande su concetti, pratica e troubleshooting
...
| 9 | Cisco IPsec (Packet Tracer) | ⭐⭐⭐ | 1.5h | IKEv1, Crypto Map, Cisco IOS |
